 ...
 * <pre>
 *  PROGRAM -> ëprogramí BLOCK ==> program
 *
 *  BLOCK -> ë{ë D* S* ë}í  ==> block
 *
 *  D -> TYPE NAME                    ==> decl
 ...
 *
 *  S -> ëifí E ëthení BLOCK ëelseí BLOCK  ==> if
 *    -> ëwhileí E BLOCK               ==> while
 *    -> ëreturní E                    ==> return
 *    -> BLOCK
 *
 *  E -> SE
...
public class Parser {
     ...
     * <
     * pre>
     * type -> 'int' type -> 'bool' tyoe -> 'ScientificN' type -> 'float'
     * </pre>
     *
     ...
     *        -> se '<=' se  ==> <= </pre> @return the tree corresponding to the
     * expression
     *
     * @exception SyntaxError - thrown for any syntax error
     */
    ...
    public AST rFactor() throws SyntaxError {
        ...
        if (isNextTok(Tokens.Minus)) {  // '-' (negation unary operator)
            t = new UnaryOpTree(currentToken);
            scan();
            t.addKid(rExpr());
            return t;
    ...
    public AST rUnary() throws SyntaxError {
        AST t, kid = rFactor();
        while ((t = getUnaryTree()) != null) {
            t.addKid(kid);
            t.addKid(rFactor());
            kid = t;
        }
    ...
    private AST getUnaryTree() {  // build tree with current token's relation
        Tokens kind = currentToken.getKind();
